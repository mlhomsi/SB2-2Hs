...
void* cria_func (void* f, DescParam params[], int n) 
{
	...
	A função irá analisar a descrição dos parâmetros passados e irá preencher um vetor com os códigos de máquina adequados para a situação.
		...
		-> Abrimos um loop controlado por n para ver todos os parâmetros
			-Dentro deste, checamos o que os parâmetros têm, preenchendo o vetor com código de máquina para criar variáveis, armazenar endereços em registrador x etc.
		
		OBS: Para descobrirmos os valores dos códigos de máquina para cada instrução, temos de programar os comandos em assembly e fazer o objdump do arquivo.o
	*/	
